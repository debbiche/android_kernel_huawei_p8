    ...
    switch (finger_status)
    {
        ...
        case FNGR_ST_LOST:
            ...
            pr_info("[FPC/TOUCH] finger lost, nav_sum_x=%d,nav_sum_y=%d, filer=%d\n",
                    fpc1020->nav.nav_sum_x, fpc1020->nav.nav_sum_y, fpc1020->nav.filter_long);
#endif
            ...
            fpc1020->nav.move_maxy = 0;
            fpc1020->nav.move_pre_dir = 0;
...
static void process_navi_event(fpc1020_data_t* fpc1020, int dx, int dy, int finger_status)
{
    ...
    const unsigned long THRESHOLD_DURATION_DTAP = 1000;
        ...
        else
        {
            if (duration < THRESHOLD_DURATION_TAP)
            {
                if (finger_status == FNGR_ST_LOST && fpc1020->nav.detect_zones != 0)
                {
                    ...
                    {
#ifdef FPC1020_NAV_DBG
                        printk("[FPC] %s: still finger lost\n", __func__);
#endif
