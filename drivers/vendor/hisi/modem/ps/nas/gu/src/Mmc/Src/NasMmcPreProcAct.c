...
)
{
    ...
    MMC_LMM_DISABLE_LTE_REASON_ENUM_UINT32  enReason;

    enReason = NAS_MML_GetDisableLteReason();
    ...
    if (NAS_MML_LTE_CAPABILITY_STATUS_DISABLE_UNNOTIFY_AS == NAS_MML_GetLteCapabilityStatus())
    {
        ...
        if (VOS_FALSE == NAS_MML_GetDisableLteRoamFlg())
        {
            ...
            /* 向LMM发送disable LTE消息 */
            NAS_MMC_SndLmmDisableLteNotify(enReason);

            if (MMC_LMM_DISABLE_LTE_REASON_LTE_ROAMING_NOT_ALLOWED == enReason)
            {
        ...
        NAS_MML_SetDisableLteReason(enReason);
    }
    else
    {
