...
void pmom_log_msg_parse(char *buf, struct pm_om_log_header *msg_head)
{
	...
	if (ptm)
	{
		...
		data_len = snprintf(&g_pmom_cat.str_buf[g_pmom_cat.str_wr], sizeof(g_pmom_cat.str_buf), 
	...
	switch (msg_magic)
	{
		...
		case PM_OM_MAGIC_WAKE:
			...
			break;
		case PM_OM_MAGIC_ADPPOWER:
			ret = adppower_msg_parse(msg_typeid, buf, msg_len, g_pmom_cat.str_buf, &g_pmom_cat.str_wr);
			break;
