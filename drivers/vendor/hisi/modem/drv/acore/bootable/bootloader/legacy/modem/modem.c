...
{
	writel(mod, (unsigned long)SHM_MEM_CHSELL_FLAG_ADDR);
	return 0;
}
...
struct pintrl_stru * get_pintrl(unsigned int mode, unsigned int ugpiog, unsigned int ugpio_bit)
{
    ...
    if(NULL == pintrl_cfg->pintrl_addr[mode].pinstu)
    {
        return NULL;
    }

    pincfg = pintrl_cfg->pintrl_addr[mode].pinstu;
...
void bsp_set_pintrl(unsigned int type, unsigned int gpio, unsigned int value)
{
    ...
    {
        case MODEM_IO_MUX:
			if(PINCONFIG_NF_VAL != lowpower_stru->iomg_val){
        ...
        case MODEM_IO_CFG:
			if(PINCONFIG_NF_VAL != lowpower_stru->iocg_val){
        ...
        case MODEM_GPIO_INPUT:
			...
			if(PINCONFIG_NF_VAL != lowpower_stru->gpio_dir){
        ...
        case MODEM_GPIO_OUTPUT:
			...
			if(PINCONFIG_NF_VAL != lowpower_stru->gpio_val){
...
int modem_pintrl_dts_parse(void *fdt)
{
    ...
    for(i = 0;i < MODEM_PIN_BUTT;i++)
    {
        ...
        pintrl_cfg->pintrl_addr[i].pinstu = cur_addr;

        memcpy(pintrl_cfg->pintrl_addr[i].pinstu, pro->data, pintrl_cfg->pintrl_size[i] * sizeof(struct pintrl_stru));

        pintrl_data_convert(pintrl_cfg->pintrl_addr[i].pinstu, pintrl_cfg->pintrl_size[i]);

    }

    modem_ldo_io_mux();

    pintrl_config(pintrl_cfg->pintrl_addr[MODEM_PIN_INIT].pinstu, pintrl_cfg->pintrl_size[MODEM_PIN_INIT]);
	#ifdef HI3630_FASTBOOT_MODEM
	fdt_pack(fdt);
	#else
...
void modem_memrepair_init(void)
{
	...
	for(i=0;i<SHM_MEM_MEMREPAIR_SIZE;){
		writel(MR_POWERON_FLAG,(unsigned long)SHM_MEM_MEMREPAIR_ADDR+i);
		i+=4;
	}
}
...
int load_modem()
{
    ...
    /*lint -save -e570*/
    writel(LOAD_MODEM_ERROR_FLAG, (unsigned long)SHM_MEM_LOADM_ADDR);
    /*lint -restore*/
	...
	img_addr = (UINT32)(head->load_addr - head_size);
	...
	#else
	if (emmc_ops->emmc_read(ptn, offset, (void *)(unsigned long)img_addr, &bytes)) {
		PRINT_ERROR("read modem fail\n");
		return -1;
	}
	#endif
    /*lint -save -e570*/
    writel(LOAD_MODEM_OK_FLAG, (unsigned long)(SHM_MEM_LOADM_ADDR));
    writel(head->load_addr, (unsigned long)(SHM_MEM_LOADM_ADDR + 4));
    writel(head->image_length, (unsigned long)(SHM_MEM_LOADM_ADDR + 8));
    /*lint -restore*/
...
extern void pm_om_smem_init(void);
    ...
    *(unsigned int*)(0xfff0a20c)=0x4050;
	...
	if(is_load_modem())
	{
		...
		if (load_modem_dsp()) {
			...
			//lint --e{570}
			writel(LOAD_DSP_ERROR_FLAG, (unsigned long)SHM_MEM_DSP_FLAG_ADDR);
			return;
		}

		PRINT_DEBUG("load modem dsp img OK\n");
		writel(LOAD_DSP_OK_FLAG, (unsigned long)SHM_MEM_DSP_FLAG_ADDR);
...
extern unsigned char binArrayEnd[];
    ...
    writel(0xA7A7A7A7, (unsigned long)SHM_MEM_LOADM_ADDR);

    modem_init();

    memcpy((void*)MCORE_TEXT_START_ADDR_COMPRESSED, binArrayStart, binArrayEnd - binArrayStart);
    ...
    do
    {
        count++;
        mdelay(1);
        value = readl((unsigned long)SHM_MEM_LOADM_ADDR);
