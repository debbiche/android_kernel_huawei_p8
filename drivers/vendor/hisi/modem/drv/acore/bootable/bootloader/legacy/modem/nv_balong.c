...
u32 bsp_nand_read(const char *partition_name, u32 partition_offset, void* ptr_ram_addr, u32 length,u32* skip_len)
{
	...
	emmc_ops = get_operators((char *)EMMC_MODULE_NAME_STR);
	if(!emmc_ops){
		cprintf("can not get emmc_ops!\n");
		return 4;
	}
	part_ops = get_operators((char *)PARTITION_MODULE_NAME_STR);
	if(!part_ops){
...
{
    s32 ret = strcmp((const char*)NV_FILE_READ,(char*)mode);
    if(B_READ  == ret)
    {
...
s32 nv_emmc_read(u8* ptr, u32 size, u32 count, FILE* fp)
{
    ...
    ret = nv_boot_read_emmc((s8*)fd->name,(fd->off+fd->seek),real_size,ptr);/*注意文件偏移*/
    if(ret != NAND_OK)
    {
...
void modem_nv_xml_decode(void)
{
    ...
    if(!nv_emmc_access((s8*)NV_XNV_CARD1_PATH,0))
    {
    ...
    if(!nv_emmc_access((s8*)NV_XNV_CARD2_PATH,0))
    {
    ...
    if(!nv_emmc_access((s8*)NV_CUST_CARD1_PATH,0))
    {
    ...
    if(!nv_emmc_access((s8*)NV_CUST_CARD2_PATH,0))
    {
