...
__ao_data u32 modem_suspend_flag = 0;
s32 modem_suspend(void)
{
    s32 ret = 0;
    modem_suspend_flag = 1;

	memcpy((void*)(PWR_TCM_MODEM_SUSPEND_ADDR), "m_suspend", sizeof("m_suspend")); /* [false alarm]:Îó±¨ */
...
__ao_func void modem_resume(void)
{
	...
	memcpy((void*)(PWR_TCM_MODEM_RESUME_ADDR), modem_string, 8); /* [false alarm]:Îó±¨ */
...
void modem_resume_ok(void)
{
	...
	memcpy((void*)(PWR_TCM_MODEM_RESUME_OK_ADDR), "m_resume_ok", sizeof("m_resume_ok")); /* [false alarm]:Îó±¨ */
    writel(bsp_get_slice_value(), PWR_TCM_MODEM_RESUME_OK_ADDR + 0x10);
    sram_resume_ok();
	cpufreq_resume();
	writel(bsp_get_slice_value(), PWR_TCM_MODEM_RESUME_OK_ADDR + 0x14);
	(void)dpm_resume();
	writel(bsp_get_slice_value(), PWR_TCM_MODEM_RESUME_OK_ADDR + 0x18);
    
    mdm_udelay(5, gNvDrxDelaySt.lpm3_3);
}

void m3_modem_sr_log(void)
{
    if(0==modem_suspend_flag)
        return;
    
    modem_suspend_flag = 0;
    
    bsp_edma_m2m_channel(0,PWR_TCM_MODEM_SUSPEND_ADDR+0x10+0xFFF20000, 
        (u32)(SHM_MEM_M3PM_LOG_ADDR+0x20000000+M3PM_LOG_MODEM_SUSPEND_OFFSET), M3PM_LOG_MODEM_SUSPEND_SIZE);
    while(bsp_edma_channel_state(0)!=0)
        {}
    
    bsp_edma_m2m_channel(0,PWR_TCM_MODEM_RESUME_ADDR+0x10+0xFFF20000, 
        (u32)(SHM_MEM_M3PM_LOG_ADDR+0x20000000+M3PM_LOG_MODEM_RESUME_OFFSET), M3PM_LOG_MODEM_RESUME_SIZE);
    while(bsp_edma_channel_state(0)!=0)
    {}
    
    bsp_edma_m2m_channel(0,PWR_TCM_MODEM_RESUME_OK_ADDR+0x10+0xFFF20000, 
        (u32)(SHM_MEM_M3PM_LOG_ADDR+0x20000000+M3PM_LOG_MODEM_RESUME_OK_OFFSET), M3PM_LOG_MODEM_RESUME_OK_SIZE);
    while(bsp_edma_channel_state(0)!=0)
    {}
    
    bsp_edma_m2m_channel(0,PWR_TCM_MODEM_DPM_ADDR+0xFFF20000, 
        (u32)(SHM_MEM_M3PM_LOG_ADDR+0x20000000+M3PM_LOG_MODEM_DPM_OFFSET), M3PM_LOG_MODEM_DPM_SIZE);
    while(bsp_edma_channel_state(0)!=0)
    {}
}
