...
__ao_data u32 m3pm_mdm_log_flag = 0;
void m3pm_log_init(void)
{
    u32 ret = 0;
    DRV_PM_OM_CFG_STRU gNvPmOmCfgSt;
    ret = bsp_nvm_read(NV_ID_DRV_PM_OM, (u8 *)&gNvPmOmCfgSt, sizeof(DRV_PM_OM_CFG_STRU));
    if(ret != NV_OK)
    {
        printk("nv read err\n");
    }
    else
    {    /* mod_sw[8]=64 bit , here,use bit PM_OM_PMM */
        m3pm_mdm_log_flag = gNvPmOmCfgSt.mod_sw[PM_OM_PMM/8]&(0x1<<(PM_OM_PMM%8));
    }
}
...
void m3_modem_updown_log(void)
{
    bsp_edma_m2m_channel(0, PWR_TCM_MODEM_DOWN_ADDR+0x10+0xFFF20000, \
        (u32)(SHM_MEM_M3PM_LOG_ADDR+0x20000000+M3PM_LOG_MODEM_DOWN_OFFSET), M3PM_LOG_MODEM_DOWN_SIZE);
    while(bsp_edma_channel_state(0)!=0)
        {}
    bsp_edma_m2m_channel(0, PWR_TCM_MODEM_UP_ADDR+0x10+0xFFF20000, \
        (u32)(SHM_MEM_M3PM_LOG_ADDR+0x20000000+M3PM_LOG_MODEM_UP_OFFSET), M3PM_LOG_MODEM_UP_SIZE);
    while(bsp_edma_channel_state(0)!=0)
        {}
}
...
void pm_modem_up(void)
{
    ...
    memcpy((void*)(PWR_TCM_MODEM_UP_ADDR), "ma9_up", sizeof("ma9_up")); /* [false alarm]:误报 */
	...
	writel(get_irq(), PWR_TCM_MODEM_UP_ADDR + 0x18);
    if(m3pm_mdm_log_flag)
    {
        m3_modem_updown_log();
        m3_modem_sr_log();
    }
	writel(bsp_get_slice_value(), PWR_TCM_MODEM_UP_ADDR + 0x1c);
...
__ao_func void check_mdma9_standbywfi(void)
{
    u32 tmp = 0;
    u32 times = 0;
	...
	do
    {
        tmp = readl(HI_SYSCTRL_BASE_ADDR + HI_SC_STAT1_OFFSET);
        tmp = (tmp >> 14) & 0x1;
        times++;
        writel(times, PWR_TCM_MODEM_DOWN_ADDR + 0x1C);
    }while(!tmp);
}
...
void pm_modem_down(void)
{
    ...
    local_irq_save(irqlock);
	memcpy((void*)(PWR_TCM_MODEM_DOWN_ADDR), "ma9_down", sizeof("ma9_down")); /* [false alarm]:误报 */
	writel(bsp_get_slice_value(), PWR_TCM_MODEM_DOWN_ADDR + 0x10);

    /* check mdma9 standbywfi */
    check_mdma9_standbywfi();
