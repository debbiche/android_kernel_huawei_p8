...
static inline void icc_pm_om_log(struct icc_channel_packet *packet, read_cb_func read_cb, void *read_context)
{
	...
	pm_om.read_cb      = (long)read_cb; /*lint !e732 */
	pm_om.read_context = (long)read_context; /*lint !e732 */

	bsp_pm_log(PM_OM_CICC, sizeof(pm_om), (void *)&pm_om);
}
