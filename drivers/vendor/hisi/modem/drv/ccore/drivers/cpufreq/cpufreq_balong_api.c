...
/*lint --e{413,570,585,628,666,732} */
...
/*lint --e{628,718,746} */
/*clear pclint Info -- Symbol '__typeof__' undeclared*/
...
static inline void cpufreq_pm_om_vote_log(int qos_id, int request_freq, int taskid, int req_id, int tar_profile)
{
	...
	bsp_pm_log_type(PM_OM_CPUF, CPUFREQ_VOTE_LOG, sizeof(struct cpufreq_pm_om_qos_log), &cpufreq_log);
}
...
int cpufreq_qos_request_with_reqid(int request_id, int request_freq)
{
	 ...
	 qos_request = (struct DFS_Qos_V7V9 *)malloc(sizeof(struct DFS_Qos_V7V9));/* [false alarm]:申请链表节点，为误报  */
	 if (NULL == qos_request)
	 {
		cpufreq_err("request:malloc failed\n");/* [false alarm]:链表节点，为误报  */
		return BSP_ERROR;
	 }
	taskid = (int)taskIdSelf();
 	local_irq_save(irqlock);
	...
	cpufreq_pm_om_vote_log(request_id, request_freq, taskid, req_id, tar_profile);
	 return req_id;
}
...
void cpufreq_print_vote_status(void)
{
	...
	list_for_each_entry(qos_list, &(g_v9_qos_list.entry), entry)
	{
		bsp_trace(BSP_LOG_LEVEL_ERROR, BSP_MUDU_CPUFREQ,"req id:%d, qos type:%d, req_value:%d ,taskid:0x%x\n", 
					qos_list->request_id, qos_list->request_type, qos_list->request_flag, qos_list->task_id);
		if (qos_list->request_type == DFS_QOS_ID_DDR_MINFREQ)
		{
