...
extern void bsp_dpm_powerdown_antn_config(void);
void pm_set_trace_level(u32 level)
{
...
#define DRX_PM_FLAG_FOR_MSP     (void*)((unsigned long)SHM_TIMESTAMP_ADDR+0x4)    /* 给msp的标志位，msp读到后马上清*/
#define DRX_PM_TIMESTAMP_ADDR   (void*)((unsigned long)SHM_TIMESTAMP_ADDR+0x100)
...
struct pm_om_info{
    ...
    u32 resume_end_time;
    u32 resume_max_time;
    u32 dpm_suspend_duration;
    u32 dpm_resume_duration;
};
struct pm_info
{
...
struct pm_info pm_ctrl_info =
{
    ...
    .pm_om.dpm_suspend_duration = 0,
    .pm_om.dpm_resume_duration = 0,
...
static void  set_a9_aux_ctrl( unsigned int bit , unsigned int bool_value)
{
/* c1 - 0 - c0 - 1 */
    unsigned int reg = 0;
...
LOCAL void scu_suspend(void)
{
    ...
    scu_ctrl_reg_bak = ARMA9CTX_REGISTER_READ(scu_reg_base+SCU_CTRL_OFFSET);/*lint !e413*/
    ARMA9CTX_REGISTER_WRITE(scu_reg_base+SCU_CTRL_OFFSET, (scu_ctrl_reg_bak&(~SCU_CTRL_CONFIG)));/*lint !e413*/
    __asm__ __volatile__ ("dsb" : : : "memory");
    __asm__ __volatile__ ("isb" : : : "memory"); 
...
LOCAL void scu_resume(void)
{
    ...
    ARMA9CTX_REGISTER_WRITE(scu_reg_base+SCU_CTRL_OFFSET, scu_ctrl_reg_bak);/*lint !e413*/
	__asm__ __volatile__ ("dsb" : : : "memory");
	__asm__ __volatile__ ("isb" : : : "memory"); 
}
...
LOCAL void sysL2CacheResume(void)
{
	...
	/* cache sync */
        ARMA9CTX_REGISTER_WRITE( l2x0_base + 0x730,0);/*lint !e539*/
...
void balong_pm_enter(suspend_state_t state)
{
    ...
    writel(pm_ctrl_info.pm_om.pm_enter_count, PM_ENTER_COUNT);
...
int pm_suspend(suspend_state_t state)/**/
{
    ...
    if(bsp_dpm_suspend())
    {
        ...
        writel(pm_ctrl_info.pm_om.dpm_fail_count, PM_DPM_FAIL_COUNT);
        return -1;
    }

    pm_timer_stamp(STAMP_AFTER_DPM_SUSPEND);
    pm_ctrl_info.pm_om.dpm_suspend_duration =get_timer_slice_delta (readl((u32)STAMP_PM_SUSPEND_START),readl((u32)STAMP_AFTER_DPM_SUSPEND) );
    ...
    pm_ctrl_info.pm_om.dpm_resume_duration =get_timer_slice_delta (readl((u32)STAMP_BEFORE_DPM_RESUME),readl((u32)STAMP_AFTER_DPM_RESUME) );
...
{
    u32 temp = 0;
    pm_ctrl_info.pm_om.suspend_start_time = readl((u32)STAMP_PM_SUSPEND_START);
    pm_ctrl_info.pm_om.suspend_end_time = readl((u32)STAMP_AFTER_SEND_IPC);
    ...
    pm_ctrl_info.pm_om.resume_start_time = readl((u32)STAMP_PWRUP_CODE_BEGIN);
    pm_ctrl_info.pm_om.resume_end_time = readl((u32)STAMP_AFTER_DPM_RESUME);
