...
int bsp_mipi_op(union mipi_ex_cmd cmd, union mipi_ex_cmd *result)
{
    ...
    spin_lock_irqsave(&mipi_ctrl_lock[cmd.attr.mipi_id], irq_flags);

	/*拉高DSP_MIPI_EN_IMI,屏蔽xbbp的其他信号*/
	set_bbp_mipi_dsp_mipi0_en_imi_dsp_mipi0_en_imi(1,(MIPI_CTRL_ENUM)cmd.attr.mipi_id);


	/*写入数据帧*/
	set_bbp_mipi_dsp_mipi0_wdata_low_dsp_mipi0_wdata_low(cmd.cmd.low_value, (MIPI_CTRL_ENUM)cmd.attr.mipi_id);
	set_bbp_mipi_dsp_mipi0_wdata_high_dsp_mipi0_wdata_high(cmd.cmd.high_value, (MIPI_CTRL_ENUM)cmd.attr.mipi_id);

	/*循环等待MIPI_GRANT_DSP为高，其为0表示正在执行任务，为1表示任务执行完毕*/
	count=0;
	reg_val= get_bbp_mipi_mipi0_grant_dsp_mipi0_grant_dsp((MIPI_CTRL_ENUM)cmd.attr.mipi_id);
	...
	while(MIPI_GRANT_DSP_FLAG_IS_HIGH != reg_val)
	{
        ...
        {
            /*拉低DSP_MIPI_EN_IMI*/
            set_bbp_mipi_dsp_mipi0_en_imi_dsp_mipi0_en_imi(0,(MIPI_CTRL_ENUM)cmd.attr.mipi_id);
            mipi_print_error("wait for MIPI_GRANT_DSP timeout: %d\n",count);
	    	spin_unlock_irqrestore(&mipi_ctrl_lock[cmd.attr.mipi_id],irq_flags);
            return MIPI_ERROR;
        }
    	reg_val= get_bbp_mipi_mipi0_grant_dsp_mipi0_grant_dsp((MIPI_CTRL_ENUM)cmd.attr.mipi_id);
	}
	...
	set_bbp_mipi_dsp_mipi0_cfg_ind_imi_dsp_mipi0_cfg_ind_imi(1,(MIPI_CTRL_ENUM)cmd.attr.mipi_id);
    ...
    else
    {
        ...
        count=0;
        reg_val= get_bbp_mipi_rd_end_flag_mipi0_soft_rd_end_flag_mipi0_soft((MIPI_CTRL_ENUM)cmd.attr.mipi_id);
        ...
        (*result).cmd.low_value=get_bbp_mipi_mipi0_rd_data_low_soft_mipi0_rd_data_low_soft((MIPI_CTRL_ENUM)cmd.attr.mipi_id);
        (*result).cmd.high_value=get_bbp_mipi_mipi0_rd_data_high_soft_mipi0_rd_data_high_soft((MIPI_CTRL_ENUM)cmd.attr.mipi_id);
		
        /*向DSP_MIPI_RD_CLR写入1，清RD_END_FLAG_MIPI_SOFT标志位*/
        set_bbp_mipi_dsp_mipi0_rd_clr_dsp_mipi0_rd_clr(1,(MIPI_CTRL_ENUM)cmd.attr.mipi_id);		
...
int bsp_mipi_r0_write(u32 mipi_id, u32 slave_id, u8 value)
{
    ...
    cmd_32.attr.parity_cmd_frame = bsp_mipi_parity_check((cmd_32.value & 0x1FFF0000) >> 16);
...
int bsp_mipi_read(u32 mipi_id, u32 slave_id, u32 reg_addr, u32 is_half_speed, u8 *value)
{
    ...
    cmd_32.attr.parity_byte_0 = bsp_mipi_parity_check(cmd_32.attr.byte_0);
