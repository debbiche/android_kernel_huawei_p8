...
/*lint -save -e10 -e26 -e64 -e718 -e746 -e413 -e628*/
static struct clk_lookup *clk_find(const char *dev_id, const char *con_id)
{
	struct clk_lookup *p=NULL, *cl = NULL;
	int match=0, best = 0;
...
/*lint -restore +e10 +e26 +e64 +e718 +e746 +e413 +e628*/
...
static int __clk_enable(struct clk *c)
{
    ...
    if (c->refcnt == 0){
        ...
        if (c->ops && c->ops->enable)
            	ret = c->ops->enable(c);
        if (ret) {
              	if (c->parent) /*lint -e830*/
              		__clk_disable(c->parent);
                return ret; /*lint -e539*/
        }
    }
    c->refcnt++;
    return 0;
