...
struct abb_info g_abbinfo;
/*om*/
static inline void abb_stamp_set(u32* stamp_addr)
{
    u32 stamp_time;
...
void bsp_abb_init(void)
{
    ...
    if (!g_abb_is_inited)
    {
        ...
        /*¿ÉÎ¬¿É²â³õÊ¼»¯*/
        g_abbinfo.om = bsp_pm_dump_get(PM_OM_ABB,sizeof(struct abb_om_info));
        if(NULL == g_abbinfo.om)
        {
            bsp_trace(BSP_LOG_LEVEL_ERROR, BSP_MODU_ABB, "pm_dump malloc failed,malloc myself!\n");
            g_abbinfo.om = &g_abbinfo.ominfo;
        }
        memset(g_abbinfo.om,0,sizeof(struct abb_om_info));
    }
...
int bsp_abb_suspend(struct dpm_device *dev)
{
    ...
    abb_stamp_set(&g_abbinfo.om->stamp_suspend[ABB_SUSPEND_IN]);
    /* read the version of ComStar */
    bsp_abb_read(0x40, &abb_version);
    abb_stamp_set(&g_abbinfo.om->stamp_suspend[ABB_SUSPEND_VER]);
    ...
    abb_stamp_set(&g_abbinfo.om->stamp_suspend[ABB_SUSPEND_END]);
...
int bsp_abb_resume(struct dpm_device *dev)
{
    ...
    abb_stamp_set(&g_abbinfo.om->stamp_resume[ABB_RESUME_IN]);
    bsp_abb_read(0x40, &abb_version);
    abb_stamp_set(&g_abbinfo.om->stamp_resume[ABB_RESUME_VER]);

    bsp_abb_init();
    abb_stamp_set(&g_abbinfo.om->stamp_resume[ABB_RESUME_INIT]);
    ...
    abb_stamp_set(&g_abbinfo.om->stamp_resume[ABB_RESUME_END]);
    return ret;
}
#else
