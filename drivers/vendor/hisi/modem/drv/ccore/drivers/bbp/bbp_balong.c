...
void bsp_bbp_timerintclear(void)
{
	...
	if(lbbp_switch){
        bbp_stamp_set(&g_stbbpinfo.om->stamp_ltetf[BBP_LTEINT_CLEAR_IN]);
	    set_hi_bbp_int_arm_int_cls_arm_position_int_cls(0x1);
        bbp_stamp_set(&g_stbbpinfo.om->stamp_ltetf[BBP_LTEINT_CLEAR_CLS]);
	}
...
int bsp_bbp_timerintenable(void)
{
	...
	lbbp_switch = bsp_bbp_get_ltemtcmos_status();
	if(lbbp_switch){
        bbp_stamp_set(&g_stbbpinfo.om->stamp_ltetf[BBP_LTEINT_EN_IN]);
		set_hi_bbp_int_arm_int_cls_arm_position_int_cls(0x1);
        bbp_stamp_set(&g_stbbpinfo.om->stamp_ltetf[BBP_LTEINT_EN_CLS]);
	    set_hi_bbp_int_arm_int_msk_arm_position_int_msk(0x1);
        bbp_stamp_set(&g_stbbpinfo.om->stamp_ltetf[BBP_LTEINT_EN_UNMASK]);
...
void bsp_bbp_timerintdisable(void)
{
	...
	lbbp_switch = bsp_bbp_get_ltemtcmos_status();
	if(lbbp_switch){
        bbp_stamp_set(&g_stbbpinfo.om->stamp_ltetf[BBP_LTEINT_DIS_IN]);
	    set_hi_bbp_int_arm_int_msk_arm_position_int_msk(0x0);
        bbp_stamp_set(&g_stbbpinfo.om->stamp_ltetf[BBP_LTEINT_DIS_MASK]);
	}
...
static void bbp_timerhandle(void)
{
	...
	lbbp_switch = bsp_bbp_get_ltemtcmos_status();
	if(lbbp_switch){
        bbp_stamp_set(&g_stbbpinfo.om->stamp_ltetf[BBP_LTEINT_HANDLE_IN]);
	    set_hi_bbp_int_arm_int_cls_arm_position_int_cls(0x1);
        bbp_stamp_set(&g_stbbpinfo.om->stamp_ltetf[BBP_LTEINT_HANDLE_CLS]);
    	bsp_ipc_spin_unlock_irqrestore(IPC_SEM_BBP,flag);
        ...
        bbp_stamp_set(&g_stbbpinfo.om->stamp_ltetf[BBP_LTEINT_HANDLE_FUNC]);
	}
	else{
    	bsp_ipc_spin_unlock_irqrestore(IPC_SEM_BBP,flag);
	}
...
int bsp_bbp_tdstf_enable(void)
{
	...
	if(tbbp_switch){
        ...
        bbp_stamp_set(&g_stbbpinfo.om->stamp_tdstf[BBP_TDSINT_EN_IN]);
	    set_hi_bbp_tstu_pub_int_msk_arm_sfrm_int_msk(0x1);
        bbp_stamp_set(&g_stbbpinfo.om->stamp_tdstf[BBP_TDSINT_EN_UNMASK]);
...
void bsp_bbp_tdstf_disable(void)
{
	...
	if(tbbp_switch){
	    ...
	    bbp_stamp_set(&g_stbbpinfo.om->stamp_tdstf[BBP_TDSINT_DIS_IN]);
	    set_hi_bbp_tstu_pub_int_msk_arm_sfrm_int_msk(0x0);
        bbp_stamp_set(&g_stbbpinfo.om->stamp_tdstf[BBP_TDSINT_DIS_MASK]);
	}
...
static void bbp_tdstf_clear(void)
{
	...
	if(tbbp_switch){
	    ...
	    bbp_stamp_set(&g_stbbpinfo.om->stamp_tdstf[BBP_TDSINT_CLEAR_IN]);
	    set_hi_bbp_tstu_pub_int_clear_arm_sfrm_int_clear(0x1);
        bbp_stamp_set(&g_stbbpinfo.om->stamp_tdstf[BBP_TDSINT_CLEAR_CLS]);
	}
...
{
    bbp_stamp_set(&g_stbbpinfo.om->stamp_tdstf[BBP_TDSINT_HANDLE_IN]);
    ...
    bbp_stamp_set(&g_stbbpinfo.om->stamp_tdstf[BBP_TDSINT_HANDLE_FUNC]);
}
/*lint -save -e801*/
...
out:
    bbp_stamp_set(&g_stbbpinfo.om->stamp_pwron[BBP_G1]);
...
out:
    bbp_stamp_set(&g_stbbpinfo.om->stamp_pwroff[BBP_G1]);
    spin_unlock_irqrestore(&bbp_power_ctrl[BBP_G1],flag);
...
out:
    bbp_stamp_set(&g_stbbpinfo.om->stamp_pwron[BBP_G2]);
...
out:
    bbp_stamp_set(&g_stbbpinfo.om->stamp_pwroff[BBP_G2]);
    spin_unlock_irqrestore(&bbp_power_ctrl[BBP_G2],flag);
...
out:
    bbp_stamp_set(&g_stbbpinfo.om->stamp_pwron[BBP_W]);
...
out:
    bbp_stamp_set(&g_stbbpinfo.om->stamp_pwroff[BBP_W]);
    spin_unlock_irqrestore(&bbp_power_ctrl[BBP_W],flag);
...
out:
    bbp_stamp_set(&g_stbbpinfo.om->stamp_pwron[BBP_TW]);
...
out:
    bbp_stamp_set(&g_stbbpinfo.om->stamp_pwroff[BBP_TW]);
    spin_unlock_irqrestore(&bbp_power_ctrl[BBP_TW],flag);
...
out:
    bbp_stamp_set(&g_stbbpinfo.om->stamp_pwroff[BBP_L]);
...
out:
    bbp_stamp_set(&g_stbbpinfo.om->stamp_pwron[BBP_L]);
	spin_unlock_irqrestore(&bbp_power_ctrl[BBP_L],flag);
...
int bsp_bbp_pwrctrl_tdsbbp_clk_enable(void)
{
    ...
    bbp_stamp_set(&g_stbbpinfo.om->stamp_clkon[BBP_T]);
...
int bsp_bbp_pwrctrl_tdsbbp_clk_disable(void)
{
    ...
    bbp_stamp_set(&g_stbbpinfo.om->stamp_clkoff[BBP_T]);
...
int bsp_bbp_pwrctrl_wbbp_clk_disable(void)
{
    ...
    bbp_stamp_set(&g_stbbpinfo.om->stamp_clkoff[BBP_W]);
...
int bsp_bbp_pwrctrl_wbbp_clk_enable(void)
{
    ...
    bbp_stamp_set(&g_stbbpinfo.om->stamp_clkon[BBP_W]);
...
static int bsp_bbp_dpm_prepare(struct dpm_device *dev)
{
    ...
    bbp_stamp_set(&g_stbbpinfo.om->stamp_suspend[BBP_PREPAIR_IN]);
#ifdef BBC_SAVE_DMA
	if(0==get_hi_crg_clkstat5_ltebbp_pd_clk_stat()){
		set_hi_crg_clken5_ltebbp_pd_clk_en(1);
		g_lbbp_clk++;
        bbp_stamp_set(&g_stbbpinfo.om->stamp_suspend[BBP_PREPAIR_CLKEN]);
	}
	/*save register data*/
	time_before = bsp_get_timer_current_value(TIMER_UDELAY_TIMER_ID);/*[false alarm]*/
	/*数据搬移的源地址*/
	...
	time_after = bsp_get_timer_current_value(TIMER_UDELAY_TIMER_ID);/*[false alarm]*/
    bbp_stamp_set(&g_stbbpinfo.om->stamp_suspend[BBP_PREPAIR_DMACFG]);
	...
	if(g_bbp_timer_debug){
		bbp_print_error("prepare begin:0x%x,prepare after:0x%x\n",time_before,time_after);/*[false alarm]*/
	}
    bbp_stamp_set(&g_stbbpinfo.om->stamp_suspend[BBP_PREPAIR_END]);
	return 0;
}
...
static int bsp_bbp_dpm_suspend(struct dpm_device *dev)
{
    ...
    bbp_stamp_set(&g_stbbpinfo.om->stamp_suspend[BBP_SUSPEND_IN]);
    ...
    bbp_stamp_set(&g_stbbpinfo.om->stamp_suspend[BBP_SUSPEND_DMAOK]);
	if(g_lbbp_clk&&(1==get_hi_crg_clkstat5_ltebbp_pd_clk_stat())){
		set_hi_crg_clkdis5_ltebbp_pd_clk_dis(1);
		g_lbbp_clk = 0;
        bbp_stamp_set(&g_stbbpinfo.om->stamp_suspend[BBP_SUSPEND_CLKDIS]);
	}
#endif
	...
	if(g_bbc_reset){
		...
		/*配置BBC复位使能*/
		set_hi_crg_srsten3_bbpcommon_2a_srst_en(1);/*[false alarm]*/

		/*配置BBC的时钟不使能*/
		set_hi_crg_clkdis5_bbpcommon_clk_dis(1);
        bbp_stamp_set(&g_stbbpinfo.om->stamp_suspend[BBP_SUSPEND_BBCRST]);
	}
...
#endif
    bbp_stamp_set(&g_stbbpinfo.om->stamp_suspend[BBP_SUSPEND_END]);
	return BSP_OK;
}
...
static int bsp_bbp_dpm_resume(struct dpm_device *dev)
{
    ...
    bbp_stamp_set(&g_stbbpinfo.om->stamp_resume[BBP_RESUME_IN]);
	time_before = bsp_get_timer_current_value(TIMER_UDELAY_TIMER_ID);/*[false alarm]*/
	if(g_bbc_reset){

		/*时钟使能*/
		set_hi_crg_clken5_bbpcommon_2a_clk_en(1);/*[false alarm]*/

		/*解复位*/
		set_hi_crg_srstdis3_bbpcommon_2a_pd_srst_dis(1);

        bbp_stamp_set(&g_stbbpinfo.om->stamp_resume[BBP_RESUME_BBCUNRST]);
	}
#ifdef BBC_SAVE_DMA
	/*DMA模式*/
	if(0==get_hi_crg_clkstat5_ltebbp_pd_clk_stat()){
	...
	*(u32 *)(bbp_ctu_save+0x420) = 0;
	...
	writel(config_data,HI_BBP_DMA_BASE_ADDR+BBP_DMA_CONFIG_OFFSET);
    bbp_stamp_set(&g_stbbpinfo.om->stamp_resume[BBP_RESUME_DMACFG]);
#endif
#ifdef BBC_SAVE_MEMCPY
	u32 i=0;
	u32 size = 0;
	...
	*(u32 *)(HI_CTU_BASE_ADDR+0x420)=g_temp_420;
    ...
    bbp_stamp_set(&g_stbbpinfo.om->stamp_resume[BBP_RESUME_END]);
	return BSP_OK;
}
...
static int bsp_bbp_dpm_complete(struct dpm_device *dev)
{
    ...
    bbp_stamp_set(&g_stbbpinfo.om->stamp_resume[BBP_COMPLETE_IN]);
	time_before = bsp_get_timer_current_value(TIMER_UDELAY_TIMER_ID);/*[false alarm]*/
    ...
    bbp_stamp_set(&g_stbbpinfo.om->stamp_resume[BBP_COMPLETE_DMAOK]);
	...
	*(u32 *)(HI_CTU_BASE_ADDR+BBP_IRM_SAVE_OFFSET) = g_bbp_irm_save;
	...
	if((1==get_hi_crg_clkstat5_ltebbp_pd_clk_stat())&&g_lbbp_clk){
		...
		g_lbbp_clk = 0;
        bbp_stamp_set(&g_stbbpinfo.om->stamp_resume[BBP_COMPLETE_CLKDIS]);
	}
#endif
...
#endif
    bbp_stamp_set(&g_stbbpinfo.om->stamp_resume[BBP_COMPLETE_END]);
	return BSP_OK;
}
...
int bbp_poweron(void)
{
	...
	/*调用G1 BBP上电流程*/
	ret |= bsp_bbp_pwrctrl_g1bbp_poweron();/*[false alarm]*/

	/*调用G2 BBP上电流程*/
	ret |= bsp_bbp_pwrctrl_g2bbp_poweron();/*[false alarm]*/

	/*调用TW BBP上电流程*/
	ret |= bsp_bbp_pwrctrl_twbbp_poweron();/*[false alarm]*/

	/*调用W BBP上电流程*/
	ret |= bsp_bbp_pwrctrl_wbbp_poweron();/*[false alarm]*/

	/*调用IRM BBP上电流程*/
	ret |= bbp_pwrctrl_irm_poweron();/*[false alarm]*/
...
int bbp_poweroff(void)
{
	...
	/*调用G1 BBP下电流程*/
	ret |= bsp_bbp_pwrctrl_g1bbp_poweroff();/*[false alarm]*/

	/*调用G2 BBP下电流程*/
	ret |= bsp_bbp_pwrctrl_g2bbp_poweroff();/*[false alarm]*/

	/*调用TW BBP下电流程*/
	ret |= bsp_bbp_pwrctrl_twbbp_poweroff();/*[false alarm]*/

	/*调用W BBP下电流程*/
	ret |= bsp_bbp_pwrctrl_wbbp_poweroff();/*[false alarm]*/
...
#endif
    /*可维可测初始化*/
    g_stbbpinfo.om = bsp_pm_dump_get(PM_OM_BBP,sizeof(struct bbp_om_info));
    if(NULL == g_stbbpinfo.om)
    {
        bbp_print_error("pm_dump malloc failed,malloc myself!\n");
        g_stbbpinfo.om = &g_stbbpinfo.ominfo;
    }
    memset(g_stbbpinfo.om,0,sizeof(struct bbp_om_info));
